			...
			<div class="container video-covid-19 pt-4" role="banner">
				...
				if ( $video_post->have_posts() ) :
					while ( $video_post->have_posts() ) :
						...
						$seriesterms = get_the_terms( $video_post->ID, 'series' );

						if ( ! empty( $seriesterms ) ) {
							// get the first term; there should only be one.
							$seriesterm = array_shift( $seriesterms );
							$seriesslug = $seriesterm->slug;
							$seriesname = $seriesterm->name;
						}
						?>

					<div class="border p-4 video-border mt-5 mb-2">
						<div class="row align-content-between justify-content-center h-100" >
							<div class="col-sm-12 col-lg-6 order-sm-1 order-md-1 order-lg-1">
								<div class="video-wrapper"><?php the_field( 'video_embed' ); ?>
								</div>
							</div>
							<div class="col-sm-12 col-lg-6 order-sm-2 order-md-2 order-lg-2  ">
								...
								<div class="content-wrapper ps-3 pc-3 d-flex flex-column align-items-start justify-content-center h-100">
									<h2 class="video-title text-center"><?php the_field( 'video_title' ); ?></h2>
									<?php the_field( 'video_description' ); ?>

										<?php
										$seriesurl = '/series/' . $seriesslug . '/'; // get the series. url.
										?>
										<div class="row flex-row w-100">\
											<div class="col-sm-6 offset-md-3 mx-auto mt-1">
											<a href="<?php echo esc_url( home_url() . '/events/' ); ?>" role="button" class="float-end btn btn-outline-primary btn-outline-primary--banner-link text-center m-0">Click HERE for more events!</a>
										</div>
										<!-- <a href="  // echo esc_url( the_field( 'button_cta_url' ) );?>" .role="button" class="btn btn-outline-primary btn-outline-primary--banner-link text-center m-0"> //echo get_field('button_cta_text');?></a>
										//else  : ?>
										<a href="  //echo home_url().'/resources/series/' . $seriesSlug; ?>" role="button" class="btn btn-outline-primary text-center m-0">  //echo $seriesName; ?></a>-->
