...
if ( ! $user ) { /* makes certain user exists */
	return;
}
?>
...
<div class="sidebar-wrapper">
	...
	role="search">
		<label class="sr-only" for="s"><?php esc_html_e( 'Search', 'workroom1128' ); ?></label>
					...
					if ( get_row_layout() === 'add_section' ) :
						...
						<div class="card">
							...
							<div class="card-header" id="<?php echo esc_html( $heading . $i ); ?>">
								<button class="list-link card-button" type="button" data-bs-toggle="collapse" data-bs-target="<?php echo esc_html( $target . $i ); ?>" aria-expanded="true" aria-controls="<?php echo esc_html( $collapse . $i ); ?>">
								<!-- Item 1 > 1  -->
								<div><?php echo esc_html( get_sub_field( 'section_name' ) ); ?></div>
								</button>
							</div>
							<div id="<?php echo esc_html( $collapse . $i ); ?>" class="collapse show" data-bs-parent="#accordion" aria-labelledby="<?php echo esc_html( $heading . $i ); ?>">
								<?php
							} else {
								?>
							<div class="card-header" id="<?php echo esc_html( $heading . $i ); ?>">
