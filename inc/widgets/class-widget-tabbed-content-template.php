<?php
/**
 * Widget Tabbed Content Template
 *
...
/**
 * Widget Tabbed Content Template Class
 */
class Widget_Tabbed_Content_Template extends WP_Widget {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'tabbed-content', home_url() . '/assets/js/tabbed-content.js', array( 'jquery' ), 'workroom1128' );
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty( $widget_title ) ) {
			echo esc_html( $args['before_title'] ) . esc_html( $widget_title ) . esc_attr( $args['after_title'] );
		}
		?>

		<div class="content clearfix">

			<?php echo esc_html( $this->render( $args, $settings ) ); ?>
	...
	public function render( $args, $settings ) {
		...
		<div class="sidebar-wrapper ">
			...
			for ( $i = 0; $i <= 3; $i++ ) :
				...
				if ( '' === $settings['tab_titles'][ $i ] && 0 === $settings['tab_content'][ $i ] ) {
					continue;
				}
				?>
				<?php
				if ( 0 === $i ) {
					...
					<li class="nav-item" role="presentation">
						<button class="nav-link active" id="<?php echo esc_html( $args['widget_id'] ); ?>-tab-<?php echo esc_html( $i ); ?>" data-bs-toggle="tab" data-bs-target="#<?php echo esc_html( $args['widget_id'] ); ?>-tabbed-<?php echo esc_html( $i ); ?>" type="button" role="tab" ><?php echo esc_html( $settings['tab_titles'][ $i ] ); ?></button>  

					</li>
				<?php } else { ?>
					<li class="nav-item" role="presentation">
						<button class="nav-link" id="<?php echo esc_html( $args['widget_id'] ); ?>-tab-<?php echo esc_html( $i ); ?>" data-bs-toggle="tab" data-bs-target="#<?php echo esc_html( $args['widget_id'] ); ?>-tabbed-<?php echo esc_html( $i ); ?>" type="button" role="tab" ><?php echo esc_html( $settings['tab_titles'][ $i ] ); ?></button>
					</li>
				<?php } ?>
				<?php endfor; ?>
			</ul>
			<div class="tab-content" id="tabbed-posts">
				...
				for ( $i = 0; $i <= 3; $i++ ) :
					?>
					<?php if ( 0 == $i ) { ?>
					<div class="tab-pane fade show active" id="<?php echo esc_html( $args['widget_id'] ); ?>-tabbed-<?php echo esc_html( $i ); ?>" role="tabpanel" aria-labelledby="<?php echo esc_html( $args['widget_id'] ); ?>-tab-<?php echo esc_html( $i ); ?>"><?php echo esc_html( $this->tab_content( $settings, $settings['tab_content'][ $i ] ) ); ?>
					</div>
				<?php } else { ?>
					<div class="tab-pane fade" id="<?php echo esc_html( $args['widget_id'] ); ?>-tabbed-<?php echo esc_html( $i ); ?>" role="tabpanel" aria-labelledby="<?php echo esc_html( $args['widget_id'] ); ?>-tab-<?php echo esc_html( $i ); ?>"><?php echo esc_html( $this->tab_content( $settings, $settings['tab_content'][ $i ] ) ); ?>
					</div>
				<?php } ?>
				<?php endfor; ?>
			</div>
	 ...
	 * @return void
	 */
	public function tab_content( $settings, $tabcontent ) {
		...
		switch ( $tabcontent ) :
			...
			<ul class="tabcontent-categories list-unstyled mt-3">
				...
				echo '<li>' . sprintf( esc_html__( '%s', 'workroom1128' ), $category_link ) . '<span class="count">(';
				//echo '<p>' . sprintf( esc_html__( 'Description: %s', 'textdomain' ), $category->description ) . '</p>';
				echo '' . sprintf( esc_html__( '%s', 'workroom1128' ), $category->count ) . ')</span></li>';
			}
			?>
			</ul>
			<?php
